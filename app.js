...
app.post('/add-vendors-ajax', function(req, res) 
{
    // Capture the incoming data and parse it back to a JS object
    let data = req.body;
...
//AJAX ADD customer FUNCTION
app.post('/add-salesInvoices-ajax', function(req, res) 
{
    // Capture the incoming data and parse it back to a JS object
    let data = req.body;
    ...
    db.pool.query(query1, function(error, rows, fields){
        ...
        {
            // If there was no error, perform a SELECT * on bsg_people
            query1 = `SELECT * FROM SalesInvoices;`;
            db.pool.query(query2, function(error, rows, fields){
...
app.put('/put-vendor-ajax', function(req,res,next){
    ...
    let queryVendor = `UPDATE Vendors SET homeworld = ? WHERE bsg_people.id = ?`;
